    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            try:
                ...
                print("fuck", self.train_filenames[index])
        elif self.mode in ['val', 'test']:
            try:
                image = Image.open(self.test_filenames[index])
                pose = self.test_poses[index]
            except:
                print("fuck", self.t6est_filenames[index])
...
def get_loader(model, image_path, metadata_path, mode, batch_size, is_shuffle=False, num_val=100):
    ...
    if mode == 'train':
        ...
        datasets = {'train': CustomDataset(image_path, metadata_path, 'train', transform, 0),
                    'val': CustomDataset(image_path,"./datasets/Cambridge Landmarks/Street/dataset_test.txt", 'test', transform)}
