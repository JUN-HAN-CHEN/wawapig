...
class Solver():
    def __init__(self, data_loader, config):
        ...
        summary(self.model.cuda(), input_size=(3, 256, 256))
        self.data_name = self.config.image_path.split('/')[-1]
        self.model_save_path = 'models_%s_%s' % (self.data_name, self.config.model)
        # self.model_save_path = 'models_NCLT_cam4_2seqs_3m'
        self.summary_save_path = 'summary_%s_%s' % (self.data_name, self.config.model)
    ...
    def train(self):
        ...
        if self.config.learn_beta:
            optimizer = optim.Adam([{'params': self.model.parameters()},
                                   ...
                                   lr=self.config.lr,
                                   weight_decay=0.0005)
            optimizer2 = optim.RMSprop([{'params': self.model.parameters()},
                                    {'params': [self.criterion.sx, self.criterion.sq]}],
                                    lr=0.045,
        ...
        scheduler = lr_scheduler.StepLR(optimizer2, step_size=1, gamma=0.98)
        ...
        for epoch in tqdm(range(start_epoch, num_epochs)):
            ...
            for phase in ['train', 'val']:
                ...
                for i, (inputs, poses) in tqdm(enumerate(data_loader)):
                    ...
                    if use_tensorboard:
                        if phase == 'train':
                            error_train.append(loss_pos_print)
                            error_train2.append(loss_ori_print)
                            ...
                            error_val.append(loss_pos_print)
                            error_val2.append(loss_ori_print)
            ...
            error_train_loss2 = np.median(error_train2)
            error_val_loss = np.median(error_val)
            error_val_loss2 = np.median(error_val2)
            ...
            print('Train and Validaion error {}, {} / {}, {}'.format(error_train_loss,error_train_loss2, error_val_loss, error_val_loss2))
            print('=' * 40)
            print('=' * 40)
    ...
    def test(self):
        ...
        self.model2 = wawapig(self.model)
        ...
        k = 0
        for i, (inputs, poses) in tqdm(enumerate(self.data_loader)):
            # print(i)
            img = inputs
            img = img.permute(0, 3, 2, 1)
            inputs = Variable(inputs, requires_grad=True).to(self.device)
            # poses = Variable(poses, requires_grad=True).to(self.device)
            # inputs = inputs.to(self.device)
            # poses = poses.to(self.device)
            _ = None
            # forward
            if self.config.bayesian:
                num_bayesian_test = 70
                ...
                for i in range(num_bayesian_test):
                    ...
                    #     ori_out = F.normalize(ori_single, p=2, dim=1)
                    #     ori_true = F.normalize(ori_true, p=2, dim=1)

                    #     loss, _, _ = self.criterion(pos_single, ori_single, pos_true, ori_true)
                    #     loss.backward()
                    #     grads = inputs.grad
                    #     grads = grads.abs()
                    ...
                    # #         plt.imshow(reverse_transform(inputs[0].detach().cpu()))
                    #         plt.axis('off')
                    #         plt.subplot(2, 1, 2)
                    #         plt.imshow(saliency, cmap=plt.cm.hot)
                    #         # plt.axis('off')
                    ...
                    #         plt.show()
                        ###
                        # print(i)
                        # pos_single2, ori_single2 = self.model2(_)
                        # ipdb.set_trace()
                        first = False
                    else:
                        pos_single, ori_single = self.model2(_)
                    # pos_single, ori_single, _ = self.model(inputs)
            ...
            #     print('{}th std: pos / ori'.format(i),pos_var, ori_var)
        ...
        # if True:
        #     f = open(self.summary_save_path + '/test.csv', 'a')
        #     f.write('{}, {},{}, {}, {}, {}, {}'.format(self.config.test_model, position_error, rotation_error, np.mean(pos_loss_arr), np.mean(ori_loss_arr), pos_var, ori_var))
        #     f.write('\n')
        #     f.close()
